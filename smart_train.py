...
same as train.py, excluding how data is selected from environment
'''
import numpy as np
...
class UAV_Env:
    ...
    def __init__(self):
        ...
        self.counter = 1
        self.BATCH_SIZE = 100
        self.action_rewards = {0: "benign", # Match actions with rewards
                    1: "portscan",
                    2: "ddos",
                    3: "bot",
                    4: "bruteforce",
        ...
        with open('./dataset/sorted/benign.csv') as file:
            self.benign = list(csv.reader(file))
        with open('./dataset/sorted/bot.csv') as file:
            self.bot = list(csv.reader(file))
        with open('dataset/sorted/bruteforce.csv') as file:
            self.bruteforce = list(csv.reader(file))
        with open('dataset/sorted/ddos.csv') as file:
            self.ddos = list(csv.reader(file))
        with open('dataset/sorted/ftp.csv') as file:
            self.ftp = list(csv.reader(file))
        with open('dataset/sorted/portscan.csv') as file:
            self.portscan = list(csv.reader(file))
        with open('dataset/sorted/sql.csv') as file:
            self.sql = list(csv.reader(file))
        with open('dataset/sorted/ssh.csv') as file:
            self.ssh = list(csv.reader(file))
        with open('dataset/sorted/xss.csv') as file:
            self.xss = list(csv.reader(file))
        with open('dataset/sorted/dos.csv') as file:
            self.dos = list(csv.reader(file))

    def get_frame(self, attack):
        if self.action_rewards[attack] == "benign":
            index = random.randint(0,len(self.benign)-1)
            frame = self.benign[index]
        elif self.action_rewards[attack] == "bot":
            index = random.randint(0,len(self.bot)-1)
            frame = self.bot[index]
        elif self.action_rewards[attack] == "bruteforce":
            index = random.randint(0,len(self.bruteforce)-1)
            frame = self.bruteforce[index]
        elif self.action_rewards[attack] == "ddos":
            index = random.randint(0,len(self.ddos)-1)
            frame = self.ddos[index]
        elif self.action_rewards[attack] == "ftp":
            index = random.randint(0,len(self.ftp)-1)
            frame = self.ftp[index]
        elif self.action_rewards[attack] == "portscan":
            index = random.randint(0,len(self.portscan)-1)
            frame = self.portscan[index]
        elif self.action_rewards[attack] == "sql":
            index = random.randint(0,len(self.sql)-1)
            frame = self.sql[index]
        elif self.action_rewards[attack] == "ssh":
            index = random.randint(0,len(self.ssh)-1)
            frame = self.ssh[index]
        elif self.action_rewards[attack] == "xss":
            index = random.randint(0,len(self.xss)-1)
            frame = self.xss[index]
        elif self.action_rewards[attack] == "dos":
            index = random.randint(0,len(self.dos)-1)
            frame = self.dos[index]
    ...
    def step(self, action): # Take action, return [reward, is_done, next_obs]
        ...
        elif self.action_rewards[action] == self.classifier: # Properly identified attack
            reward = 10
        else:
       ...
       # print("action: ", self.action_rewards[action])

        # Check if done
        if (self.counter == self.BATCH_SIZE): # Made it to end of data
